...
public:
	...
	void zigzag_1(int count);
                            ...
                            // óâåëè÷åíèå ñêîðîñòè ïàäåíèÿ ôèãóðû
...
void Tetris:: output()
{
	...
	cout << endl << endl << "   Î×ÊÈ: " << score << endl;
}
                            // çàâåðøåíèå èãðû
void Tetris:: end()
{
	system("cls");
	cout << endl << endl << endl << endl << endl << "              ÂÛ ÇÀÂÅÐØÈËÈ ÈÃÐÓ :)" << endl << endl;
	cout << endl << endl << endl << endl << endl << "                  âàøè î÷êè: " << score << endl << endl << endl << endl << endl;
	exit(0);
}

                            /* ìåòîäû äëÿ ôèãóð (êâàäðàò, ëèíèÿ, äâà âèäà óãëîâ, çèãçàã è ïèðàìèäêà)
                               äëÿ êàæäîé ôèãóðû â çàâèñèìîñòè îò íàæàòîé êëàâèøè 
							   ïðîïèñàíû âîçìîæíûå ïîëîæåíèÿ ïîÿâëåíèÿ è ïðè ïîâîðîòå, à òàê æå âîçìîæíîñòè ïåðåìåùåíèÿ
							*/
	...
	while (true)
	{
		...
		if ((view && (pole[e][a] == el_fig || pole[e + 1][b] == el_fig || pole[e + 1][c] == el_fig || e + 1 == m - 1)) || (!view && (pole[c + 1][d] == el_fig || pole[c][e] == el_fig || c + 1 == 0)))
			break;
                      ...
                      /* ñàìà èãðà
					     êàæäîé ôèãóðå ñîîòâåòñòâóåò ÷èñëî, ïðîãðàììà íà êàæäîì ýòàïå ãåíåðèðóåò ðàíäîìíîå ÷èñëî è ñîçäàåò ñîîòâåòñòâóþùóþ ôèãóðó
						 åñëè âñÿ ãîðèçîíòàëüíàÿ ëèíèÿ çàïîëíåíà, óäàëÿåì åå, à îñòàëüíûå ñìåùàåì âíèç
                      */
void Tetris:: game()
{
	int a, b, c, d, e;
...
void Tetris:: begin()
{
	...
	Sleep(1300);
	cout << endl << "    ÄÎÁÐÎ ÏÎÆÀËÎÂÀÒÜ Â ÈÃÐÓ :)" << endl;
	Sleep(1300);
	cout << endl << " äëÿ óïðàâëåíèÿ ôèãóðàìè èñïîëüçóéòå ñòðåëî÷êè" << endl;
	Sleep(1300);
	cout << endl << " äëÿ ïåðåâîðà÷èâàíèÿ ôèãóð èñïîëüçóéòå space" << endl;
	Sleep(1300);
	cout << endl << " äëÿ çàâåðøåíèÿ èãðû íàæìèòå Escape" << endl;
	Sleep(1300);
	cout << endl << " äëÿ íà÷àëà èãðû íàæìèòå Enter" << endl;
	int input;
	input = _getch();
	switch (input)
	{
